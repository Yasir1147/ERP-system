...
class OfficeStaffController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        $data = $request->validate([
            ...
            'attendance_mode' => ['sometimes', Rule::in(['sessions', 'fixed'])],
            'fixed_start_time' => ['required_if:attendance_mode,fixed', 'nullable', 'date_format:H:i'],
            'fixed_end_time' => ['required_if:attendance_mode,fixed', 'nullable', 'date_format:H:i', 'after:fixed_start_time'],
        ...
        DB::transaction(function () use ($data, $photoPath) {
            ...
            OfficeStaff::create([
                ...
                'status' => $data['status'],
                'attendance_mode' => $data['attendance_mode'] ?? 'sessions',
                'fixed_start_time' => $data['fixed_start_time'] ?? '09:00',
                'fixed_end_time' => $data['fixed_end_time'] ?? '17:00',
            ]);
        });
