...
class PayrollController extends Controller
{
    ...
    public function payslipExport(Request $request, Employee $employee): StreamedResponse
    {
        ...
        return $this->csvDownload('payslip-'.$employee->id.'-'.$month->format('Y-m').'.csv', [
            ...
            [],
            ['Days', 'Per Day', 'Salary', 'OT Hrs', 'OT Salary', 'New Total', 'Bonus', 'Pr. Balance', 'Total Balance', 'Deduction', 'Paid Cash', 'Balance', 'Remarks'],
            [
                $row['presentDays'],
                $row['dailySalary'],
                $row['basicSalary'],
    ...
    public function ledgerExport(Request $request, Employee $employee): StreamedResponse
    {
        ...
        $csvRows = [
            ...
            [],
            ['Month', 'Days', 'Per Day', 'Salary', 'OT Hrs', 'OT Salary', 'New Total', 'Bonus', 'Pr. Balance', 'Total Balance', 'Deduction', 'Paid Cash', 'Balance', 'Remarks'],
        ];
    ...
    private function payrollRows(string $monthStart, string $monthEnd, string $month, ?string $type, ?int $employeeId): Collection
    {
        ...
        return $employees->map(function (Employee $employee) use ($records, $adjustments, $month) {
    ...
    private function payrollRowForMonth(Employee $employee, Collection $employeeRecords, ?PayrollAdjustment $adjustment, float $autoPreviousBalance): array
    {
    ...
    private function carryForwardBalance(Employee $employee, Carbon $targetMonth): float
    {
        ...
        foreach (CarbonPeriod::create($firstMonth, '1 month', $targetMonth) as $month) {
            ...
            $balance = $this->payrollRowForMonth($employee, $records, $adjustment, $balance)['balance'];
        }
    ...
    private function firstPayrollMonth(Employee $employee): ?Carbon
    {
        ...
        $dates = collect([$firstAttendanceDate, $firstAdjustmentMonth])->filter();
