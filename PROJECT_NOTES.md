...
- Errors with no field of their own on the duty screen — `duty_date`, `plan`, `target_date`, `assignment` — are shown in the page-level red banner. The duty date is picked in step 1 and has no input in step 2, so a date outside the user's allowed range previously rejected the request with nothing at all appearing on screen.
...
- That section opens on the dashboard's own selected date and switches to All recent with one click. Its date picker is the dashboard's own date filter, so changing it moves the whole page to that day. The selected date's duties are queried separately from the recent thirty, so paging back to an older day still shows that day's duties.
...
- The Projects and Employees tiles on each duty card reveal the full list on hover or keyboard focus: projects with their head count, employees with their project and status. The counts stay readable at a glance and the detail costs no click. The popup's offset is its own padding rather than a margin — a real gap drops the hover the moment the cursor leaves the tile — and it lingers briefly on leave so the cursor can reach it.
- Pages that sit outside the admin layout and are reached by an attendance account — Mark Attendance and both Contracting Duty screens — carry their own account line with the signed-in name, email, and a Log out button. They have no sidebar, so without it a field user cannot tell whose account they are on or leave it.
...
- Step 2 and 3 carry a worker search that filters every project accordion at once. Duties run to a dozen projects, so finding one person meant opening each accordion in turn; searching leaves only the matching rows and forces those groups open. It uses the same forgiving matcher as the employee dropdown, so near spellings still match.
- Copied duty text starts with the duty date, because the message is read in WhatsApp away from the screen that produced it.
- The review step offers Finish Planning and a green Submit Attendance side by side. There is no Save Draft button; leaving the page already keeps the plan as a draft.
- Employee search in the duty workspace tolerates spelling differences, so `rahul` finds `RUHUL AMIN`, `mohammad` finds `MUHAMMAD`, and `bilal` finds `BILLAL`. Worker names arrive through inconsistent transliteration, and an exact search hid people who were really in the list. Matching lives in `resources/js/lib/employee-search.ts` and runs in the browser against the already-loaded list, so there is no extra request. Terms of three characters or fewer match exactly, because at that length almost anything is within one edit.
